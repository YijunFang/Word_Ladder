...
class Lexicon {
 ...
 public:
	...
	void mapAll(void (fn)(std::string word, ClientDataType &), ClientDataType &data);
...
#define BYTE_ORDER LITTLE_ENDIAN
    ...
    Edge *traceToLastEdge(const std::string & s);
    void readBinaryFile(std::string filename);
    template <typename ClientDataType>
	void recMapAll(Edge *edge, bool first, std::string soFar, void (fn)(std::string, ClientDataType &), ClientDataType &data);
...
template <typename ClientDataType>
void Lexicon::recMapAll(Edge *edge, bool first, std::string soFar, void (fn)(std::string word, ClientDataType &), ClientDataType &clientData)
{
    if (!edge) return;
...
template <typename ClientDataType>
void Lexicon::mapAll(void (fn)(std::string word, ClientDataType &), ClientDataType &clientData)
{
    recMapAll(start, true, "", fn, clientData);	// map over dawg
    for (std::set<std::string>::iterator itr = otherWords.begin(); itr != otherWords.end(); itr++)
